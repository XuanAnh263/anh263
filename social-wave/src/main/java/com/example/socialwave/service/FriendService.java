...
//    }
    ...
    public void sendFriendRequest(UserRequest userRequest, FriendRequest friendRequest) {
        ...
        User friend = userRepository.findByUsername(friendRequest.getUsername())
        ...
        for (Friend request : existingRequests) {
            if (request.getFriend().getUsername().equals(friend.getUsername())) {
                if (request.getStatus() == FriendStatus.PENDING) {
                    throw new IllegalArgumentException("Friend request already sent to this user");
                } else if (request.getStatus() == FriendStatus.ACCEPTED) {
                    throw new IllegalArgumentException("Were friends");
                } else if (request.getStatus() == FriendStatus.DENIED) {
                    throw new IllegalArgumentException("Friend request denied");
                }
            }
        }
        boolean isAlreadyFriend = existingRequests.stream()
                .anyMatch(existingFriend -> existingFriend.getFriend().getId().equals(friend.getId()));
    ...
    public void deleteFriend(Long id, User authenticateUser) {
        Friend friend = friendRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("Friend not found"));
        if (!friend.getUser().getId().equals(authenticateUser.getId())) {
